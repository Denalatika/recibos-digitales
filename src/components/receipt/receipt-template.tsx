...
export const ReceiptTemplate: React.FC<ReceiptTemplateProps> = ({
  ...
  return (
    <div 
      className={`receipt-sheet bg-white text-slate-800 shadow-sm print:shadow-none border border-slate-300 print:border-slate-400 rounded-lg overflow-hidden mx-auto font-sans w-full ${className}`}
      style={{
        width: '100%',
    ...
    >
      ...
      <div className="receipt-header relative flex flex-row items-stretch justify-between bg-white border-b border-slate-300 min-h-[72px]">
        ...
        >
          <div className="flex items-center space-x-3">
            ...
            <div className="min-w-0">
              <h1 className="text-white font-black text-sm md:text-base tracking-wider uppercase leading-tight">
                {company.business_name || company.name || 'EMPRESA DEMO'}
              </h1>
              {company.slogan && (
                <p 
                  className="font-bold text-[9px] tracking-widest uppercase mt-0.5"
        ...
        <div className="px-4 py-2 w-[310px] shrink-0 bg-white flex flex-col justify-center text-right">
          ...
          <div className="grid grid-cols-2 gap-x-2 gap-y-0.5 text-left text-[10px] leading-snug">
      ...
      <div className="p-3 space-y-2.5">
        ...
        <div className="border-b border-slate-200 pb-2">
          <div className="flex items-center justify-between mb-1">
            ...
            {person.status === 'archived' && (
              <span className="text-[8px] bg-slate-100 text-slate-500 px-1.5 py-0.5 rounded font-medium">
                Archivado
              </span>
            )}
          </div>

          <div className="grid grid-cols-2 gap-x-6 gap-y-1 text-[10px] leading-normal">
            {/* Columna Izquierda */}
            <div className="space-y-0.5">
              <div className="flex items-baseline">
                <span className="w-24 font-extrabold text-slate-900 uppercase shrink-0">NOMBRE:</span>
                <span className="font-bold text-slate-900 flex-1 truncate">{person.full_name || '-'}</span>
              </div>
              <div className="flex items-baseline">
                <span className="w-24 font-extrabold text-slate-900 uppercase shrink-0">NÚMERO INT:</span>
                <span className="font-semibold text-slate-700 flex-1">{person.internal_id || '-'}</span>
              </div>
              <div className="flex items-baseline">
                <span className="w-24 font-extrabold text-slate-900 uppercase shrink-0">PUESTO:</span>
            ...
            <div className="space-y-0.5">
              <div className="flex items-baseline">
                <span className="w-24 font-extrabold text-slate-900 uppercase shrink-0">DEPTO:</span>
                <span className="font-semibold text-slate-700 flex-1 truncate">{person.department || '-'}</span>
              </div>
              <div className="flex items-baseline">
                <span className="w-24 font-extrabold text-slate-900 uppercase shrink-0">R.F.C.:</span>
                <span className="font-semibold text-slate-700 flex-1">{person.rfc || '-'}</span>
              </div>
              <div className="flex items-baseline">
                <span className="w-24 font-extrabold text-slate-900 uppercase shrink-0">RÉGIMEN:</span>
        ...
        <div className="grid grid-cols-12 gap-2.5 items-start">
          ...
          <div className="col-span-5 rounded border border-slate-200 overflow-hidden flex flex-col justify-between min-h-[105px]">
            <div>
              <div 
                className="text-white font-black uppercase tracking-wider px-2.5 py-1 text-[10px]"
              ...
              <table className="w-full text-[10px]">
                <thead>
                  <tr className="border-b border-slate-200 text-slate-600 font-bold bg-slate-50">
                    ...
                    <th className="py-1 px-1.5 text-center">REF</th>
                ...
                <tbody className="divide-y divide-slate-100">
                  {earnings.map((e, idx) => (
                    <tr key={e.id || idx}>
                      <td className="py-0.5 px-2 font-medium text-slate-800 truncate max-w-[130px]">{e.concept}</td>
                      <td className="py-0.5 px-1.5 text-[9px] text-center text-slate-500">{e.reference || '-'}</td>
                      <td className="py-0.5 px-2 text-right font-semibold text-slate-800 whitespace-nowrap">
            ...
            <div className="bg-slate-50 border-t border-slate-200 flex items-center justify-between font-black px-2.5 py-1 text-[10px]">
          ...
          <div className="col-span-4 rounded border border-slate-200 overflow-hidden flex flex-col justify-between min-h-[105px]">
            <div>
              <div 
                className="text-white font-black uppercase tracking-wider px-2.5 py-1 text-[10px]"
              ...
              <table className="w-full text-[10px]">
                ...
                <tbody className="divide-y divide-slate-100">
                  {deductions.map((d, idx) => (
                    <tr key={d.id || idx}>
                      <td className="py-0.5 px-2 font-medium text-slate-800 truncate max-w-[110px]">{d.concept}</td>
                      <td className="py-0.5 px-1.5 text-[9px] text-center text-slate-500">{d.reference || '-'}</td>
                      <td className="py-0.5 px-2 text-right font-semibold text-slate-800 whitespace-nowrap">
          ...
          <div className="col-span-3 rounded border-2 border-slate-300 flex flex-col justify-between bg-slate-50/70 p-2.5 min-h-[105px]">
            <div className="text-center">
              <span className="text-[9px] font-black text-slate-700 tracking-wider uppercase">
                NETO A PAGAR
              </span>
              <div 
                className="font-black tracking-tight text-lg md:text-xl mt-0.5"
                style={{ color: '#0f766e' }}
              >
            ...
            <div className="border-t border-slate-200 pt-1.5 mt-1 space-y-0.5 text-[9px] leading-snug">
        ...
        <div className="border-t border-slate-200 pt-2 grid grid-cols-12 gap-3 items-center">
          ...
          <div className="col-span-5 space-y-1 text-[9px]">
            ...
            <div className="grid grid-cols-2 bg-slate-50 rounded border border-slate-200 p-1.5 gap-1 text-[9px] leading-snug">
              <div>
                <span className="text-slate-500 font-bold uppercase block text-[8px]">MÉTODO</span>
                <p className="font-bold text-slate-800 truncate">Depósito Bancario</p>
              </div>
          ...
          <div className="col-span-4 flex items-center bg-slate-50 rounded border border-slate-200 p-1.5 space-x-2">
            ...
            <div className="text-[9px] leading-snug space-y-0.5 flex-1 min-w-0">
              <span className="font-black text-slate-900 uppercase block text-[9px]">VALIDACIÓN</span>
              <p className="text-slate-500 truncate text-[8px]">
              ...
              <div className="pt-0.5">
                <span className={`inline-flex items-center space-x-1 px-1.5 py-0.5 rounded font-extrabold text-[8px] border ${currentStatus.color}`}>
                  {currentStatus.icon}
      ...
      <div 
        className="px-4 py-1.5 text-[9px] text-white font-medium"
        style={{ backgroundColor: primaryColor }}
      >
